...
public class MyScrollView extends ScrollView implements View.OnTouchListener {
	...
	public void loadMoreImages() {
		if (hasSDCard()) {
			...
			if (startIndex < CosActivity.picList.size()) {
				Toast.makeText(getContext(), "正在加载...", Toast.LENGTH_SHORT)
						.show();
				if (endIndex > CosActivity.picList.size()) {
					endIndex = CosActivity.picList.size();
				}
				for (int i = startIndex; i < endIndex; i++) {
					LoadImageTask task = new LoadImageTask();
					taskCollection.add(task);
					task.execute(CosActivity.picList.get(i));
				}
				page++;
			} else {
	...
	class LoadImageTask extends AsyncTask<String, Void, Bitmap> {
		...
		private Bitmap loadImage(String imageUrl) {
			...
			if (!imageFile.exists()) {
				Log.e("!imageFile.exists()", String.valueOf(!imageFile.exists()));
		...
		private void downloadImage(String imageUrl) {
			...
			if (imageFile != null) {
				...
				Bitmap bitmap = ImageLoader.decodeSampledBitmapFromResource(
						imageFile.getPath(), columnWidth);
				Log.e("download", String.valueOf(bitmap)+imageUrl);
				if (bitmap != null) {
					imageLoader.addBitmapToMemoryCache(imageUrl, bitmap);
				}
